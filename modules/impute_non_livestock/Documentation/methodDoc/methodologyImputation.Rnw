...
A cricial aspect of this process consists in the identification of figures that have to be considered  reliable enough and not overwritten by the imputation process. Those figures are recognzed by their flag combination (Observation Flag, Method flag) and have been defined \textbf{Protected figures}. The complete list of \textit{protected} flag combination is managed by the \code{faoswsFlag} sub-module which contains the \textit{flagValidTable}\footnote{ Here an extraction of the \code{flagValidTable} is reported the following table reports only those flag combination thare will not be overwritten by the module(\textbf{protected figures}):

\newpage

<<Flag, eval=TRUE, echo=FALSE, dev='png'>>=
library(faoswsFlag)
library(data.table)
print(faoswsFlag::flagValidTable[Protected==TRUE])
...
for(iter in seq(selectedItemCode)){
            ...
            extractedData =
                GetData(subKey)
            extractedData=preProcessing(extractedData )
           
            
            extractedData
            extractedData=extractedData[geographicAreaM49 %in% c("835","381","716","894","704","858","804","800","792","788","764","752","724","705","703","686","688","616","566","458","422","398","36","76","894","634",
"620", "604", "528", "466", "398", "300", "288")]
...
<<wheat-yield-explore,fig=TRUE,  echo = FALSE>>=
 extractedData=denormalise(extractedData, denormaliseKey = "measuredElement")
 library(lattice)

 
            extractedData=nameData("agriculture",
                                   "aproduction",
                                   extractedData,
                                   except = colnames(extractedData)[colnames(extractedData)!="geographicAreaM49"]
                                   )

##test=extractedData[geographicAreaM49=="380"]
extractedData=extractedData[order(extractedData$geographicAreaM49_description),]

xyplot(Value_measuredElement_5421 ~ timePointYears|geographicAreaM49_description, data = extractedData, type = c("g", "l"), xlab = "", ylab = "Wheat Yield (1000 Hg/Ha)")
@
...
<<wheat-production-area-explore, fig=TRUE, echo = FALSE>>=
xyplot(Value_measuredElement_5510 + Value_measuredElement_5312 ~ timePointYears|geographicAreaM49_description,
       data = extractedData, type = c("g", "l"),
       xlab = "", ylab = "",
       scales = list(y = list(relation = "free", draw = FALSE)),
       auto.key =  list(text = c("Production", "Area harvested")))
@
