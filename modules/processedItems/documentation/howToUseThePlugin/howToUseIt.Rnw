...
\Abstract{
  ...
  This document has been written to ensure that each user is able to autonomously run the plugin. The first section  explains how to set the requested parameters and clarifies the meaning of each requested parameter.
The second section makes reference to those data tables (stored in the SWS) containing, for each dimension, the complete imputation key. It supports the users in case the complete imputation key has to be enlarged or reduced. It basically gives to the user the possibility to change the complete set of data the module has to work on, without modifying the \textbf{R} routines.

The last section presents some intermediate files produced by the plugin and stored in a shared folder. Those files (both csv and pdf plots) have been created to support the validation process of the output.
  \\
...
Choosing the \textit{DerivedItems_Imputation}, within the list of available plugins, the user is asked to set the basic parameters of the sub-module in order to define the imputation key for the plugin execution.
...
Thanks to this parameter the user can choose if apply the plugin to the countries already contained in the session ("\textit{Country in session}") or to produce imputations for all the country ("\textit{All countries}"). In this latter case the session will be enlarged to include all the FBS countries stored in the data table FBS_countries.
...
\item{The \textit{recovery} folder contains some intermediate csv files that are not useful for validation purposes. These files are stored in the shared folder to be used just in case the module fails and it is necessary to force the restarting.}
\end{itemize}
...
The output is save back to the Agriculture Production dataset. In particular only the 5510 corresponding to the PRODUCTION expressed in tons is saved in the session.
